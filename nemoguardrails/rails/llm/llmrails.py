        ...
        try:
            ...
            if options:
                messages = [
                    {
                        "role": "context",
                        "content": {"generation_options": options.dict()},
                    }
                ] + messages
            ...
            # If you're using a streaming handler, ensure it's properly closed.
            streaming_handler = streaming_handler_var.get()
            if streaming_handler:
                await streaming_handler.push_chunk(None)

            # Depending on your use case, you can:
            # 1. Re-raise the exception to let the caller handle it:
            raise
