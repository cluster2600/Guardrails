            ...
            for future in asyncio.as_completed(tasks):
                try:
                    ...
                    if is_blocked:
                        ...
                        # check if any other tasks completed while we were processing
                        # this reports the race condition where multiple tasks might block
                        for task in tasks:
                            if task.done() and not task.cancelled():
                                try:
                                    other_flow_id, other_is_blocked = task.result()
                                    if other_is_blocked and other_flow_id != flow_id:
                                        log.warning(
                                            f"Multiple rails blocked content: {flow_id} and {other_flow_id}. "
                                            f"Only {flow_id} will be reported."
                                        )
                                except Exception:
                                    pass
