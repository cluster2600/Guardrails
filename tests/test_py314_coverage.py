...
class TestEagerTaskFactory:
    """Test that execute() correctly installs/restores the eager task factory."""

    @pytest.mark.asyncio
    async def test_execute_restores_task_factory(self):
        """After execute(), the original task factory should be restored."""
    ...
    @pytest.mark.asyncio
    async def test_execute_restores_factory_on_error(self):
        """Even if a rail raises, the task factory should be restored."""
