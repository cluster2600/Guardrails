...
try:
    ...
    if llm_task_manager.config.rails.config.jailbreak_detection.nim_url is not None:
...
@pytest.mark.skipif(
    ...
    reason="JailbreakDetect NIM not running or endpoint is not in config.",
)
def test_jb_detect_nim_unsafe():
