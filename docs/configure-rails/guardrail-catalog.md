...
The following languages are available for multilingual language detection. Upon refusal, the library returns the refusal message in the detected language.
...
If the detected language is not in this list, it falls back to the default refusal message in English.

##### Enabling Multilingual Language Detection

To enable multilingual language detection, install the NeMo Guardrails library with the `multilingual` extra:

```bash
pip install nemoguardrails[multilingual]
```

This extra installs the [fast-langdetect package](https://github.com/LlmKira/fast-langdetect), which detects the language of user input so the library can return refusal messages in the appropriate language.

##### Enabling Multilingual Refusal Messages

To enable multilingual refusal messages, add the `multilingual` configuration to your `config.yml` as shown in the following example.
...
##### Specifying Custom Refusal Messages

You can specify custom refusal messages for each language as shown in the following example.
...
If a custom message is not provided for a detected language, it falls back to the default refusal message for that language.

##### Cold Start Behavior

The fast-langdetect library downloads a language detection model on first use.
For more details on first call behavior, refer to the [fast-langdetect documentation](https://github.com/LlmKira/fast-langdetect?tab=readme-ov-file#memory-note).

##### Production Considerations

- First API call might take 10-20 seconds to download and load the model, depending on the network.
- Subsequent calls use the cached model with ~9-12μs latency.
- For container or serverless environments, pre-warm during startup or persist the model cache in your container image.

##### Language Detection Accuracy Benchmark

Language detection is benchmarked on the following datasets.
