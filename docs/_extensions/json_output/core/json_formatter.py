...
class JSONFormatter:
    ...
    def add_metadata_fields(self, data: dict[str, Any], metadata: dict[str, Any]) -> None:
        ...
        New schema: topics, tags, content.type, content.difficulty, content.audience, facets.modality
        ...
        # Difficulty: content.difficulty (new) or difficulty (legacy)
        difficulty = content.get("difficulty") if isinstance(content, dict) else None
        difficulty = difficulty or metadata.get("difficulty")
        if difficulty:
