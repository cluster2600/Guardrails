...
Frontmatter fields supported (new nested schema):
- description: Page description for meta tags
- topics: Primary domains/topics for filtering
- tags: Keywords for SEO
- content.type: Type of content (Tutorial, Concept, Reference, How-To, etc.)
- content.difficulty: Content difficulty level (Beginner, Intermediate, Advanced)
- content.audience: Target audience list (Technical Writer, Developer, etc.)
- facets.modality: Content modality (text-only, image-only, multimodal, etc.)

Legacy fields still supported for backwards compatibility:
- personas → maps to content.audience
- difficulty → maps to content.difficulty
- content_type → maps to content.type
- modality → maps to facets.modality
- cascade.product.name/version → from conf.py
"""
...
def _get_audience(metadata: dict[str, Any]) -> list[str] | None:
    """Extract audience from new or legacy schema."""
    # New schema: content.audience
    content = metadata.get("content", {})
    if isinstance(content, dict) and content.get("audience"):
        audience = content["audience"]
        return audience if isinstance(audience, list) else [audience]

    # Legacy schema: personas
...
def _add_audience_tag(metadata: dict[str, Any]) -> list[str]:
    ...
    audience = _get_audience(metadata)
    if not audience:
        return []

    # Map legacy persona keys to readable names
    audience_map = {
    ...
    audiences = [audience_map.get(p, p) for p in audience]
...
def _get_content_type(metadata: dict[str, Any]) -> str | None:
    """Extract content type from new or legacy schema."""
    # New schema: content.type
    content = metadata.get("content", {})
    if isinstance(content, dict) and content.get("type"):
        return content["type"]
    # Legacy schema: content_type
    return metadata.get("content_type")


def _get_difficulty(metadata: dict[str, Any]) -> str | None:
    """Extract difficulty from new or legacy schema."""
    # New schema: content.difficulty
    content = metadata.get("content", {})
    if isinstance(content, dict) and content.get("difficulty"):
        return content["difficulty"]
    # Legacy schema: difficulty
    return metadata.get("difficulty")


def _get_facets(metadata: dict[str, Any]) -> dict[str, Any]:
    """Extract all facets from frontmatter (dynamic, user-defined keys)."""
...
def _get_modality(metadata: dict[str, Any]) -> str | None:
    """Extract modality from new or legacy schema."""
    # New schema: facets.modality
    facets = _get_facets(metadata)
    if facets.get("modality"):
        return facets["modality"]
    # Legacy schema: modality
...
def _add_custom_fields(metadata: dict[str, Any]) -> list[str]:
    ...
    Supports both new nested schema and legacy flat fields.
    Dynamically extracts all facet keys.
    """
    custom_tags = []

    # Add audience tags (content.audience or legacy personas)
    custom_tags.extend(_add_audience_tag(metadata))

    # Add topics if present (new schema)
    ...
    # Add content type (content.type or legacy content_type)
    content_type = _get_content_type(metadata)
    if content_type:
        custom_tags.append(f'<meta name="content-type-category" content="{content_type}">')

    # Add difficulty (content.difficulty or legacy difficulty)
    difficulty = _get_difficulty(metadata)
    if difficulty:
        custom_tags.append(f'<meta name="difficulty" content="{difficulty}">')

    # Dynamically add all facets as meta tags
    facets = _get_facets(metadata)
    for facet_key, facet_value in facets.items():
        if facet_value:
            # Handle both string and list values
            if isinstance(facet_value, list):
                facet_str = ", ".join(str(v) for v in facet_value)
            else:
                facet_str = str(facet_value)
            custom_tags.append(f'<meta name="facet-{facet_key}" content="{facet_str}">')

    # Legacy flat modality support (if not in facets)
    if "modality" not in facets and metadata.get("modality"):
...
def build_json_ld(metadata: dict[str, Any], context: dict[str, Any]) -> str | None:  # noqa: C901, PLR0912
    ...
    # Add topics as additional keywords
    if "topics" in metadata and isinstance(metadata["topics"], list):
        existing = structured_data.get("keywords", [])
        structured_data["keywords"] = existing + metadata["topics"]

    # Add content type mapping (content.type or legacy content_type)
    content_type = _get_content_type(metadata)
    if content_type:
        type_mapping = {
            "tutorial": "HowTo",
    ...
    # Add difficulty as proficiency level (content.difficulty or legacy difficulty)
    difficulty = _get_difficulty(metadata)
    if difficulty:
        difficulty_map = {
            "beginner": "Beginner",
        ...
        if difficulty in difficulty_map:
            structured_data["proficiencyLevel"] = difficulty_map[difficulty]

    # Add audience (content.audience or legacy personas)
    audience = _get_audience(metadata)
    if audience:
        audience_map = {
            "data-scientist-focused": "Data Scientists",
    ...
    # Add product information from cascade (legacy) or conf.py
